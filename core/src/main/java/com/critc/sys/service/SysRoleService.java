    ...
    public String createMenuStr(int role_id) {
        ...
        if (menu == null) {
            ...
            for (SysResource sysResource : listResource) {
                if (sysResource.getParentId() == 0 && displayResourceIdList.contains(sysResource.getId())) {
                    ...
                    for (SysResource sysResourceChild : listResource) {
                        if (sysResourceChild.getParentId() == sysResource.getId()
                            ...
                            sb.append("<li id=\"module_" + sysResourceChild.getId() + "\" class=\"nav-item start \">\n" +
                                    " <a href=\"" + pubConfig.getDynamicServer() + "/" + sysResourceChild.getUrl() + "\" class=\"nav-link\" target=\"" + sysResourceChild.getTarget() + "\">\n" +
                                    " <i class=\"fa " + sysResourceChild.getIconImg() + "\"></i>\n" +
                                    " <span class=\"title\">" + sysResourceChild.getName() + "</span>\n" +
                                    " </a>\n" +
                                    " </li>");
                        }
                    }
